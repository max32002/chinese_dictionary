...
class ChineseComponentSearch {
  ...
  async performSearch() {
    ...
    try {
      const response = await fetch(
        `api.php?action=search&keyword=${encodeURIComponent(keyword)}`
      );
  ...
  displayResults(results, keyword) {
    ...
    results.forEach((result, index) => {
      ...
      card.addEventListener("click", () =>
        this.showCharacterDetail(result.char, result.data)
      );
  ...
  async showCharacterDetail(char, data = null) {
    ...
    if (!data) {
      try {
        const response = await fetch(
          `api.php?action=detail&char=${encodeURIComponent(char)}`
        );
  ...
  generateDetailHTML(char, data) {
    const formatList = (arr) => {
      ...
      return arr
        .map((item) => `<span class="pronunciation-tag">${item}</span>`)
        .join("");
    };
    ...
    const formatVariants = (arr) => {
      ...
      return arr
        .map(
          (item) => `<span class="variant-char" title="點擊查看">${item}</span>`
        )
        .join("");
    };
    ...
    return `
            <div class="char-detail">
                ...
                <div class="char-unicode">U+${data.unicode_hex || "N/A"} (${
      data.unicode || "N/A"
    })</div>
  ...
  async decomposeInput() {
    ...
    try {
      const response = await fetch(
        `api.php?action=decompose&text=${encodeURIComponent(input)}`
      );
  ...
  async copyResults() {
    ...
    try {
      ...
      this.showToast(
        `已複製 ${this.currentResults.length} 個字到剪貼簿`,
        "success"
      );
    } catch (err) {
